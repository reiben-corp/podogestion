...
def reset_y_seed():
    ...
    try:
        ...
        # Reiniciar secuencias (solo si no hay datos)
        if db.query(User).count() == 0:
            db.execute(text("ALTER SEQUENCE users_id_seq RESTART WITH 1"))
        else:
            max_id = db.query(User).order_by(User.id.desc()).first().id
            db.execute(text(f"ALTER SEQUENCE users_id_seq RESTART WITH {max_id + 1}"))
        
        if db.query(Paciente).count() == 0:
            db.execute(text("ALTER SEQUENCE pacientes_id_seq RESTART WITH 1"))
        else:
            max_id = db.query(Paciente).order_by(Paciente.id.desc()).first().id
            db.execute(text(f"ALTER SEQUENCE pacientes_id_seq RESTART WITH {max_id + 1}"))
        ...
        for i, (nombre, apellidos) in enumerate(NOMBRES, 1):
            paciente = Paciente(
                ...
                profesion=random.choice(["Médico", "Ingeniero", "Profesional", "Jubilado", "Estudiante", "Comerciante"]),
                ...
                contacto_emergencia=f"{random.choice(['María', 'Juan', 'Pedro', 'Laura'])} {apellidos.split(' ')[0]}",
                telefono_emergencia=f"6{random.randint(10000000, 99999999)}",
                alergias=random.choice([None, "Penicilina", "Yodo", "Látex", None, None]),
                medicacion_actual=random.choice([None, "Metformina 850mg", "Sintrom", "Enalapril 5mg", None, None]),
                antecedentes=random.choice([None, "HTA controlada", "DM tipo 2", "Cirugía rodilla 2020", None, None]),
                diabetes=random.choice([True, False, False, False]),
                problemas_cardiovasculares=random.choice([True, False, False, False]),
                problemas_coagulacion=random.choice([True, False, False, False]),
                enfermedades_reumaticas=random.choice([True, False, False, False]),
                enfermedades_neurologicas=random.choice([True, False, False, False]),
                enfermedades_oseas=random.choice([True, False, False, False]),
                hepatitis_vih=random.choice([True, False, False, False]),
                embarazada=random.choice([True, False, False, False, False, False]),
                cirugias_previas=random.choice([None, "Cirugía tobillo 2019", "Cirugía rodilla 2020", None, None]),
                traumatismos_pies=random.choice([True, False, False, False]),
                plantillas_previas=random.choice([True, False, False, False]),
                deporte=random.choice([True, False, False, False]),
                frecuencia_deporte=random.choice(["3x semana", "1x semana", "Diario", None, None]),
                tipo_calzado=random.choice(["Deportivo", "Vestir", "Bota", "Tacón", None, None]),
                horas_pie_dia=random.choice([4, 6, 8, 10, None, None]),
                fumador=random.choice([True, False, False, False]),
        ...
        # Citas hoy (mayormente confirmadas para el dashboard)
        for i in range(3):
            cita = Cita(
        ...
        # Citas mañana (confirmadas para el dashboard)
        for i in range(2):
            cita = Cita(
                paciente_id=random.choice(pacientes).id,
                profesional_id=random.choice(profesionales_obj).id,
